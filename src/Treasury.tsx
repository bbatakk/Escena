...
import { concertSettlement, createId, formatDate, formatMoney, totalMerchRevenue, totalNetConcertFees, type Concert, type LabelAgreement, type MerchSale, type MoneyMovement, type MoneyMovementKind } from './model'
...
export default function Treasury({ concerts, labelAgreement }: { concerts: Concert[]; labelAgreement: LabelAgreement | null }) {
  ...
  const merchTotal = totalMerchRevenue(concerts, merchSales)
  const settlements = concerts.map((concert) => concertSettlement(concert, labelAgreement))
  const netFees = totalNetConcertFees(concerts, labelAgreement)
  const labelCommission = settlements.filter((item) => !item.unresolved).reduce((sum, item) => sum + item.paidCommission, 0)
  const pendingFees = settlements.filter((item) => item.unresolved && item.grossPaid > 0)
  const income = movements.filter((item) => item.kind === 'ingres').reduce((sum, item) => sum + item.amount, 0)
  const totalIncome = income + merchTotal + netFees
  ...
  return <div className="treasury-shell"><div className="page-heading treasury-heading"><div><span className="eyebrow">CONTROL INTERN</span><h1>Tresoreria<span className="heading-period">.</span></h1><p>Registra els diners que entren i surten de la banda, amb o sense concert.</p></div></div>
    <div className="money-summary"><div><span className="eyebrow">BALANÇ TOTAL</span><strong className={totalIncome - expenses >= 0 ? 'positive-money' : 'negative-money'}>{formatMoney(totalIncome - expenses)}</strong><small className="summary-note">Ingressos totals menys despeses</small></div><div><span className="eyebrow">INGRESSOS TOTALS</span><strong className="positive-money">{formatMoney(totalIncome)}</strong><small className="summary-note">Catxets nets {formatMoney(netFees)} · Moviments {formatMoney(income)} · Marxandatge {formatMoney(merchTotal)}</small></div><div><span className="eyebrow">DESPESES</span><strong className="negative-money">{formatMoney(expenses)}</strong></div></div>
    <div className="treasury-merch-note"><span className="eyebrow">CATXETS NETS INCLOSOS AL BALANÇ</span><strong>{formatMoney(netFees)}</strong><span>Comissions de discogràfica sobre el que s’ha cobrat: {formatMoney(labelCommission)}. Els catxets ja s’hi sumen automàticament; no els registris com a ingressos manuals.</span>{pendingFees.length ? <span>Hi ha {pendingFees.length} {pendingFees.length === 1 ? 'concert cobrat' : 'concerts cobrats'} sense gestió definida: {formatMoney(pendingFees.reduce((sum, item) => sum + item.grossPaid, 0))} bruts encara no inclosos al balanç. Indica qui els ha gestionat a les fitxes.</span> : null}</div>
    <div className="treasury-merch-note"><span className="eyebrow">MARXANDATGE INCLOS AL BALANÇ</span><strong>{formatMoney(merchTotal)}</strong><span>Inclou vendes detallades i resums antics sense vendes desglossades. No les tornis a registrar com a moviment manual.</span></div>

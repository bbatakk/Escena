...
import { deleteMoneyMovement, listMoneyMovements, saveMoneyMovement } from './data'
...
export default function Treasury({ concerts }: { concerts: Concert[] }) {
  ...
  useEffect(() => { let active = true; listMoneyMovements().then((items) => { if (active) setMovements(items) }).catch((cause) => { if (active) setError(cause instanceof Error ? cause.message : 'No s’han pogut carregar els moviments.') }).finally(() => { if (active) setLoading(false) }); return () => { active = false } }, [])
  ...
  return <div className="treasury-shell"><div className="page-heading treasury-heading"><div><span className="eyebrow">CONTROL INTERN</span><h1>Tresoreria<span className="heading-period">.</span></h1><p>Registra els diners que entren i surten de la banda, amb o sense concert.</p></div></div>
    <div className="money-summary"><div><span className="eyebrow">BALANÇ REGISTRAT</span><strong className={income - expenses >= 0 ? 'positive-money' : 'negative-money'}>{formatMoney(income - expenses)}</strong></div><div><span className="eyebrow">INGRESSOS</span><strong className="positive-money">{formatMoney(income)}</strong></div><div><span className="eyebrow">DESPESES</span><strong className="negative-money">{formatMoney(expenses)}</strong></div></div>

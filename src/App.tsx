...
function HomeView({ concerts, onOpen, onNewConcert, onGoToConcerts, onGoToCalendar }: { concerts: Concert[]; onOpen: (id: string) => void; onNewConcert: () => void; onGoToConcerts: () => void; onGoToCalendar: () => void }) {
  ...
  return <div className="home-dashboard">
    ...
    {next ? <section className="home-next-gig"><div className="home-next-date"><span>{formatDate(next.date, { weekday: 'short' }).replace('.', '').toUpperCase()}</span><strong>{next.date.slice(8, 10)}</strong><small>{formatDate(next.date, { month: 'short' }).replace('.', '').toUpperCase()}</small></div><div className="home-next-info"><span className="eyebrow">PROPER CONCERT <i /> {statusLabels[next.status].toUpperCase()}</span><h2>{next.title}</h2><p><MapPin size={15} /> {concertPlace(next) || 'Ubicació per concretar'}</p></div><button type="button" className="button button-light" onClick={() => onOpen(next.id)}>Obrir fitxa <ArrowRight size={16} /></button></section> : <section className="home-next-gig home-next-empty"><div className="home-next-date"><Music2 size={23} /></div><div className="home-next-info"><span className="eyebrow">PROPER CONCERT</span><h2>Encara no hi ha cap data</h2><p>Afegeix un concert per començar a preparar la temporada.</p></div><button type="button" className="button button-light" onClick={onNewConcert}><Plus size={16} /> Crear concert</button></section>}
...
function Detail({ concert, labelAgreement, onBack, onEdit, onDelete, onToggle, onUpload, onRemoveFile }: { concert: Concert; labelAgreement: LabelAgreement | null; onBack: () => void; onEdit: () => void; onDelete: () => void; onToggle: (id: string) => Promise<void>; onUpload: (id: string, file: File) => Promise<void>; onRemoveFile: (id: string) => Promise<void> }) {
  ...
  useEffect(() => {
    ...
    const closeWithEscape = (event: KeyboardEvent) => { if (event.key === 'Escape') setMaterialOpen(false) }
    const previousOverflow = document.body.style.overflow
    document.body.style.overflow = 'hidden'
    window.addEventListener('keydown', closeWithEscape)
    return () => { document.body.style.overflow = previousOverflow; window.removeEventListener('keydown', closeWithEscape) }
  ...
  return <div className="detail-shell">
    ...
    <div className="detail-body"><div className="detail-main">
      ...
      <section className="detail-section"><div className="detail-section-heading"><FileText size={19} /><h2>Documents</h2></div>{d.documents.length ? <div className="document-list">{d.documents.map((doc) => <div className="document-item" key={doc.id}><span className="document-icon"><FileText size={17} /></span><div><strong>{doc.name || 'Document sense nom'}</strong><small>{doc.direction === 'enviar' ? 'Per enviar' : 'Per rebre'} · {doc.status === 'pendent' ? 'Pendent' : doc.status === 'no_cal' ? 'No cal' : doc.direction === 'enviar' ? 'Enviat' : 'Rebut'}</small>{doc.storagePath ? <div className="document-file"><Paperclip size={13} />{documentLinks[doc.id] ? <a href={documentLinks[doc.id]} target="_blank" rel="noreferrer">{doc.fileName || 'Obrir fitxer adjunt'}</a> : documentLinks[doc.id] === '' ? <span>No s'ha pogut obrir el fitxer.</span> : <span>{doc.fileName || 'Fitxer adjunt'} · preparant enllaç…</span>}{cloudConfigured ? <button type="button" disabled={busyDocument !== null} onClick={() => void removeFile(doc.id)}>Treure</button> : null}</div> : null}{cloudConfigured ? <label className="document-upload">{busyDocument === doc.id ? 'Pujant fitxer…' : doc.storagePath ? 'Substituir fitxer' : 'Adjuntar fitxer'}<input type="file" disabled={busyDocument !== null} onChange={(e) => { const file = e.target.files?.[0]; void upload(doc.id, file); e.target.value = '' }} /></label> : null}</div>{safeLink(doc.url) ? <a href={safeLink(doc.url)!} target="_blank" rel="noreferrer" aria-label={`Obrir enllaç de ${doc.name}`}><ExternalLink size={16} /></a> : null}</div>)}</div> : <p className="section-empty">Encara no hi ha documents registrats. Afegeix-los editant la fitxa.</p>}{documentError ? <p className="form-error" role="alert">{documentError}</p> : null}</section>
       ...
       <section className="detail-section material-launch"><div className="detail-section-heading"><PackageCheck size={19} /><div><h2>Material a portar</h2><p>Marca què ja està carregat abans de sortir.</p></div></div><div className="material-launch-row"><div><strong>{d.materials.filter((item) => item.loaded).length}/{d.materials.length}</strong><span>elements carregats</span></div><button type="button" className="button button-primary" onClick={() => setMaterialOpen(true)}><PackageCheck size={16} /> Obrir material</button></div></section>
        ...
        <section className="detail-section merch-sale-launch"><div className="detail-section-heading"><span className="quick-sale-heading-icon"><ShoppingBag size={19} /></span><div><h2>Marxandatge</h2><p>Registra una venda i resta l’estoc al moment.</p></div></div><div className="merch-sale-launch-row"><div><strong>{formatMoney(concertMerchRevenue)}</strong><span>{concertSales.reduce((sum, item) => sum + item.quantity, 0)} unitats venudes en aquest concert</span></div><button type="button" className="button button-primary" onClick={() => { setSaleError(''); setMerchSaleOpen(true) }}><ShoppingBag size={16} /> Obrir venda</button></div></section>

  ...
  return <div className="detail-shell">
    ...
    <div className="detail-body"><div className="detail-main">
      ...
      <section className="detail-section"><div className="detail-section-heading"><PackageCheck size={19} /><h2>Material a portar</h2><span className="section-counter">{d.materials.filter((item) => item.loaded).length}/{d.materials.length} carregat</span></div>{d.materials.length ? <div className="material-list">{d.materials.map((item) => <label className={`material-item ${item.loaded ? 'is-loaded' : ''}`} key={item.id}><input type="checkbox" checked={item.loaded} disabled={busyMaterial} onChange={() => void toggleMaterial(item.id)} /><span className="check-visual"><Check size={14} /></span>{item.name || 'Material sense nom'}</label>)}</div> : <p className="section-empty">Afegeix material a la fitxa per preparar la càrrega.</p>}{materialError ? <p className="form-error" role="alert">{materialError}</p> : null}</section>
       ...
       <section className="detail-section merch-quick-sale"><div className="detail-section-heading quick-sale-heading"><span className="quick-sale-heading-icon"><ShoppingBag size={19} /></span><div><h2>Venda ràpida</h2><p>Un toc per cada producte venut.</p></div><strong>{formatMoney(concertSales.reduce((sum, item) => sum + item.quantity * item.unitPrice, 0))}</strong></div>{merchProducts.filter((item) => item.active).length ? <div className="quick-sale-grid">{merchProducts.filter((item) => item.active).map((product) => { const sold = merchSales.filter((item) => item.productId === product.id).reduce((sum, item) => sum + item.quantity, 0); const sizes = product.sizes || []; return <div className="quick-sale-product" key={product.id}><strong>{product.name}</strong><span>{formatMoney(product.price)}</span>{sizes.length ? <div className="quick-sale-sizes">{sizes.map((size) => { const sizeSold = merchSales.filter((item) => item.productId === product.id && item.size === size.name).reduce((sum, item) => sum + item.quantity, 0); const key = `${product.id}:${size.name}`; return <button type="button" key={size.name} disabled={savingProductId === key || sizeSold >= size.stock} onClick={() => void quickSale(product, size.name)}>{size.name}<small>{sizeSold}/{size.stock}</small></button> })}</div> : <><small>{sold}/{product.stock} venudes</small><button type="button" className="quick-sale-add" disabled={savingProductId === `${product.id}:` || sold >= product.stock} onClick={() => void quickSale(product)}>{savingProductId === `${product.id}:` ? '…' : '+1'}</button></>}</div> })}</div> : <p className="section-empty">Afegeix productes a Marxandatge per activar la venda ràpida.</p>}{saleError ? <p className="form-error" role="alert">{saleError}</p> : null}{concertSales.length ? <div className="quick-sale-history">{concertSales.slice(0, 8).map((sale) => <div key={sale.id}><span>+{sale.quantity} · {merchProducts.find((item) => item.id === sale.productId)?.name || 'Producte eliminat'}{sale.size ? ` · ${sale.size}` : ''}</span><strong>{formatMoney(sale.quantity * sale.unitPrice)}</strong><button type="button" className="text-button" disabled={undoingSaleIds.includes(sale.id)} onClick={() => void undoSale(sale)}>{undoingSaleIds.includes(sale.id) ? '…' : 'Desfer'}</button></div>)}</div> : null}</section>

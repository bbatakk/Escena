...
function Detail({ concert, onBack, onEdit, onDelete, onToggle, onUpload, onRemoveFile }: { concert: Concert; onBack: () => void; onEdit: () => void; onDelete: () => void; onToggle: (id: string) => Promise<void>; onUpload: (id: string, file: File) => Promise<void>; onRemoveFile: (id: string) => Promise<void> }) {
  ...
  async function quickSale(product: MerchProduct) { const sold = merchSales.filter((item) => item.productId === product.id).reduce((sum, item) => sum + item.quantity, 0); if (sold >= product.stock) { setSaleError(`No queda estoc de ${product.name}.`); return } setSavingProductId(product.id); setSaleError(''); try { const saved = await saveMerchSale({ id: createId(), concertId: concert.id, productId: product.id, quantity: 1, unitPrice: product.price, note: '' }); setMerchSales((items) => [saved, ...items]) } catch (cause) { setSaleError(cause instanceof Error ? cause.message.replace('No hi ha prou estoc disponible', `No queda prou estoc de ${product.name}`).replace('Producte de marxandatge no trobat', 'No s’ha trobat el producte.') : 'No s’ha pogut registrar la venda.') } finally { setSavingProductId((id) => id === product.id ? null : id) } }
  ...
  return <div className="detail-shell">
    ...
    <div className="detail-body"><div className="detail-main">
      ...
      <section className="detail-section"><div className="detail-section-heading"><PackageCheck size={19} /><h2>Material a portar</h2><span className="section-counter">{d.materials.filter((item) => item.loaded).length}/{d.materials.length} carregat</span></div>{d.materials.length ? <div className="material-list">{d.materials.map((item) => <label className={`material-item ${item.loaded ? 'is-loaded' : ''}`} key={item.id}><input type="checkbox" checked={item.loaded} disabled={busyMaterial} onChange={() => void toggleMaterial(item.id)} /><span className="check-visual"><Check size={14} /></span>{item.name || 'Material sense nom'}</label>)}</div> : <p className="section-empty">Afegeix material a la fitxa per preparar la càrrega.</p>}{materialError ? <p className="form-error" role="alert">{materialError}</p> : null}</section>
       ...
       <section className="detail-section merch-quick-sale"><div className="detail-section-heading quick-sale-heading"><span className="quick-sale-heading-icon"><ShoppingBag size={19} /></span><div><h2>Venda ràpida</h2><p>Un toc per cada producte venut.</p></div><strong>{formatMoney(concertSales.reduce((sum, item) => sum + item.quantity * item.unitPrice, 0))}</strong></div>{merchProducts.filter((item) => item.active).length ? <div className="quick-sale-grid">{merchProducts.filter((item) => item.active).map((product) => { const sold = merchSales.filter((item) => item.productId === product.id).reduce((sum, item) => sum + item.quantity, 0); return <button type="button" className="quick-sale-product" key={product.id} disabled={savingProductId === product.id || sold >= product.stock} onClick={() => void quickSale(product)}><strong>{product.name}</strong><span>{formatMoney(product.price)}</span><small>{sold}/{product.stock} venudes</small><b>{savingProductId === product.id ? '…' : '+1'}</b></button> })}</div> : <p className="section-empty">Afegeix productes a Marxandatge per activar la venda ràpida.</p>}{saleError ? <p className="form-error" role="alert">{saleError}</p> : null}{concertSales.length ? <div className="quick-sale-history">{concertSales.slice(0, 8).map((sale) => <div key={sale.id}><span>+{sale.quantity} · {merchProducts.find((item) => item.id === sale.productId)?.name || 'Producte eliminat'}</span><strong>{formatMoney(sale.quantity * sale.unitPrice)}</strong><button type="button" className="text-button" disabled={undoingSaleIds.includes(sale.id)} onClick={() => void undoSale(sale)}>{undoingSaleIds.includes(sale.id) ? '…' : 'Desfer'}</button></div>)}</div> : null}</section>

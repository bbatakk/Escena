...
const BandResources = lazy(() => import('./BandResources'))
type Screen = 'list' | 'calendar' | 'detail' | 'form' | 'library' | 'treasury' | 'merch' | 'resources'
  ...
  return <div className="app-layout">
    ...
    <main className="main-area"><header className="topbar"><button type="button" className="icon-button menu-trigger" aria-label="Obrir menú" onClick={() => setMenuOpen(true)}><Menu size={21} /></button><span className="topbar-path">Espai de la banda <span>/</span> {screen === 'calendar' ? 'Calendari' : screen === 'detail' ? 'Fitxa del concert' : screen === 'form' ? 'Editar fitxa' : screen === 'library' ? 'Documents' : screen === 'treasury' ? 'Tresoreria' : screen === 'merch' ? 'Marxandatge' : screen === 'resources' ? 'Recursos' : 'Concerts'}</span><span className="topbar-right">{cloudConfigured ? (online ? 'EN LÍNIA' : 'SENSE CONNEXIÓ') : 'DEMO LOCAL'} <span className={`online-dot ${online ? '' : 'offline-dot'}`} /></span></header>
      <div className="content-area">
        ...
        {screen === 'resources' ? <Suspense fallback={<div className="content-loading">Carregant recursos…</div>}><BandResources /></Suspense> : null}

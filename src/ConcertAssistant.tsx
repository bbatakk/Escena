...
function makeConcertDraft(value: DraftResponse): Concert {
  ...
  concert.country = text(value.country)
...
export default function ConcertAssistant({ concerts, onCreateDraft }: { concerts: Concert[]; onCreateDraft: (draft: Concert) => void }) {
  ...
  async function ask(event: FormEvent<HTMLFormElement>) {
    ...
    const context = concerts.map((concert) => ({ title: concert.title, date: concert.date, status: statusLabels[concert.status], venue: concert.venue, city: concert.city, country: concert.country, feeAmount: concert.feeAmount, feePaid: concert.feePaid, pending: getPending(concert) }))
    const now = new Date()

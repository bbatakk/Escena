...
17. Per activar l’assistent amb el nivell gratuït de Gemini, crea una clau a Google AI Studio i desa-la com a secret amb `supabase secrets set GEMINI_API_KEY=...`. Executa també `supabase/migrations/202609240014_ai_daily_limit.sql` i desplega `supabase functions deploy concert-assistant` (torna a desplegar-la després de canviar-ne el codi). L’assistent respon preguntes i fa anàlisis de concerts i imports, prepara esborranys, proposa edicions i eliminacions de concerts, i crea, edita o arxiva plantilles de repertori. Els canvis es previsualitzen i requereixen confirmació; els concerts nous passen pel formulari. Les eliminacions de concerts amb vendes o moviments vinculats es bloquegen. La IA no escriu directament a la base de dades. Les peticions envien a Gemini el text, dades de concerts, horaris, repertoris i imports, però no contactes ni notes de les fitxes. El límit de l’app és de 10 peticions per compte i dia; Google aplica les seves pròpies quotes gratuïtes. Opcionalment configura `GEMINI_MODEL`; per defecte és `gemini-3.6-flash`. No posis mai la clau al codi web ni a `.env.local`.

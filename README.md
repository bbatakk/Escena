...
17. Per activar l’assistent amb el nivell gratuït de Gemini, crea una clau a Google AI Studio i desa-la com a secret amb `supabase secrets set GEMINI_API_KEY=...`. Executa també `supabase/migrations/202609240014_ai_daily_limit.sql` i desplega `supabase functions deploy concert-assistant` (torna a desplegar-la després de canviar-ne el codi). L’assistent respon preguntes i fa anàlisis; prepara concerts; proposa crear, editar o arxivar persones, material, documents, productes i plantilles; gestiona moviments de tresoreria, vendes de marxandatge, el nom de la banda i el tema visual local. També permet editar camps de la fitxa, incloses les persones que hi van; els documents es poden eliminar. Pujar fitxers, importar backups i marcar el checklist físic de material es fa des de la interfície. Les accions es previsualitzen i requereixen confirmació; els concerts nous passen pel formulari. Les eliminacions de concerts amb vendes o moviments vinculats es bloquegen. La IA no escriu directament a la base de dades. Les peticions envien a Gemini el text, dades de concerts, noms i rols de persones, material, documents, productes, vendes i moviments de tresoreria (incloses les notes dels moviments), però no telèfons ni correus guardats de persones. El límit de l’app és de 10 peticions per compte i dia; Google aplica les seves pròpies quotes gratuïtes. Opcionalment configura `GEMINI_MODEL`; per defecte és `gemini-3.6-flash`. No posis mai la clau al codi web ni a `.env.local`.
18. Per configurar una discogràfica opcional per banda, executa `supabase/migrations/202609240016_band_label_commission.sql` **abans** de publicar la versió web que en fa ús. A Configuració indica nom i trams; cada concert guarda una còpia de les condicions quan selecciones «Discogràfica» com a gestor. El catxet cobrat és brut; el net cobrat es calcula amb el tram que correspon al total rebut i se suma automàticament als ingressos de Tresoreria (no el registris també com a ingrés manual). Els concerts antics sense gestor en una banda amb discogràfica s’han de classificar a la fitxa per poder-ne incloure el net al balanç. Els backups nous inclouen la configuració i els antics continuen sent importables. Després, torna a desplegar `concert-assistant` si fas servir les consultes de catxets de l’IA.

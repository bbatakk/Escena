...
type ConcertSummary = {
  ...
  schedule?: unknown[]
  setlist?: string
  details?: { dinner?: string; lodging?: string; lodgingAddress?: string; personIds?: string[] }
}
type GeminiResponse = {
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (action === 'parse_offer') {
      ...
      const referenceDate = typeof body.referenceDate === 'string' && /^\d{4}-\d{2}-\d{2}$/.test(body.referenceDate) ? body.referenceDate : new Date().toISOString().slice(0, 10)
      const concerts = Array.isArray(body.concerts) ? (body.concerts as ConcertSummary[]).slice(0, 300) : []
      const safeConcerts = concerts.map((item) => ({ id: item.id, title: item.title, date: item.date, status: item.status, statusLabel: item.statusLabel, venue: item.venue, city: item.city, country: item.country, address: item.address, feeAmount: item.feeAmount, feePaid: item.feePaid, pending: Array.isArray(item.pending) ? item.pending : [], schedule: Array.isArray(item.schedule) ? item.schedule : [], setlist: item.setlist || '', details: { dinner: item.details?.dinner, lodging: item.details?.lodging, lodgingAddress: item.details?.lodgingAddress, personIds: Array.isArray(item.details?.personIds) ? item.details.personIds : [] } }))
